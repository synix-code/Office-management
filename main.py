...
class DeleteScreen(Screen):
    ...
    def populate_main_list(self, dt):
        ...
        if self.employee_details == []:
            print("hi")
            self.ids.delete_spinner.active = False
            self.ids.empty_delete.opacity = 1
        
        else:

            try:
                self.ids.delete_spinner.active = True
                for name, role, salary in self.employee_details:
                    ...
                    self.ids.delete_list.add_widget(item)
                ...
                self.ids.delete_list.add_widget(item_empty)
                self.ids.empty_delete.opacity = 0
    ...
    def item_clicked(self, instance):
        ...
        self.manager.current = "dele"
        fillatt_screen = self.manager.get_screen("dele")
        fillatt_screen.ids.dele_label.text = instance.text

        # self.ids.user_att.text = instance.text


class DeleScreen(Screen):
    pass
...
class ArrivalScreen(Screen):
    ...
    def populate_main_list(self, dt):
        ...
        if self.employee_details == []:
            ...
            self.ids.empty_arrival.opacity = 1
        
        else:

            try:
                self.ids.arrival_spinner.active = True
                for name, role, salary in self.employee_details:
                    item = TwoLineListItem(
                        text=name,
                        secondary_text=f"Role: {role}"
                ...
                self.ids.empty_arrival.opacity = 0
...
class DepartureScreen(Screen):
    ...
    def populate_main_list(self, dt):
        ...
        if self.employee_details == []:
            ...
            self.ids.empty_departure.opacity = 1
        
        else:

            try:
                self.ids.departure_spinner.active = True
                for name, role, salary in self.employee_details:
                    item = TwoLineListItem(
                        text=name,
                        secondary_text=f"Role: {role}"
                ...
                self.ids.empty_departure.opacity = 0
...
class MyApp(MDApp):
    ...
    def on_start(self):
        ...
        check_thread = threading.Thread(target=self.check_updates)
        check_thread.start()
    ...
    def check_updates(self):
        try:
            ...
            doc_ref = db.collection('updates').document('streetcoding777@gmail.com')
            doc = doc_ref.get()

            if doc.exists:
                data = doc.to_dict()
                
                # Check for 'notice' field and print notice string if 'notice' is True
                if data.get('notice', False):
                    print("Notice:", data.get('notice_str', 'No notice available'))
                    notice_str = data.get("notice_str", "No notice available")
                    notie = "Notice!"
                    Clock.schedule_once(lambda dt: self.show_startup_dialog(notice_str, notie), 0)

                # Check for 'version' field
                current_version = '1.0.0'  # Replace with your actual app version
                latest_version = data.get('version', '0.0.0')
                if latest_version > current_version:
                    print("Update Available! New Version:", latest_version)
                    noti = "Update Notice!"
                    noti_str = data.get("version_str", "No version available")
                    Clock.schedule_once(lambda dt: self.show_startup_dialog(noti_str, noti), 0)
                    if data.get('day', 1) == 0:
                        print("Day is 0, exiting script.")
                        Clock.schedule_once(self.exit_app, 0)# Exit the script if day = 0
    ...
    def show_startup_dialog(self, notice_str, notice, dt=None):
    ...
    def exit_app(self, dt):
        # Forcefully exit the app (this is done in main thread)
    ...
    def call_attendence_download(self):
        self.bldr.get_screen("view").ids.dndspinner.active = True
        thread = threading.Thread(target=self.generate_attendance_pdf)
        thread.start()
    ...
    def generate_attendance_pdf(self):
        try:
            ...
            def draw_header():
                ...
                c.drawString(margin, height - 85, f"Company: {comp}")
            ...
            # ✅ Save PDF to memory (no physical file)
            c.save()

            # ✅ Send email with PDF attachment
            self.send_email_with_attachment(pdf_buffer, email, comp)

            

            self.bldr.get_screen("view").ids.dndspinner.active = False
            Clock.schedule_once(lambda dt: self.download_dialog(), 0)

        except Exception as e:
            print(f"Error as : {e}")


    def send_email_with_attachment(self, pdf_buffer, recipient_email, comp_name):
        try:
            sender_email = config.Email
            sender_password = config.Password
            ...
            msg['Subject'] = f"Attendance Report for {comp_name}"

            # Attach PDF file
            part = MIMEBase('application', 'octet-stream')
            part.set_payload(pdf_buffer.getvalue())
            encoders.encode_base64(part)
            part.add_header('Content-Disposition', f'attachment; filename="{comp_name}-Attendance.pdf"')
            msg.attach(part)
    ...
    def download_dialog(self):
        ...
        self.dialog = MDDialog(
            ...
            text=f"Your PDF has been sent to {email} download it.",
            size_hint=(0.8, None),
        )
        self.dialog.open()


    def call_inward_download(self):
        self.bldr.get_screen("view").ids.dndspinner.active = True
        thread = threading.Thread(target=self.send_inward_pdf_email)
        thread.start()
    ...
    def send_inward_pdf_email(self):
        try:
            # ✅ Load user data
            with open("user_data.json", "r") as f:
                user_data = json.load(f)
    ...
    def call_outward_download(self):
        self.bldr.get_screen("view").ids.dndspinner.active = True
        thread = threading.Thread(target=self.send_outward_pdf_email)
        thread.start()
    ...
    def send_outward_pdf_email(self):
        try:
            # ✅ Load user data
            with open("user_data.json", "r") as f:
                user_data = json.load(f)
            ...
            doc_ref = db.collection("users").document(email)
            doc = doc_ref.get()
            inward = doc.to_dict().get("outward", {})
            ...
            def draw_header():
                c.setFont("Helvetica-Bold", 24)
                c.setFillColor(pink)
                c.drawString(margin, height - 50, "OutWard Entry")
                c.setStrokeColor(pink)
                c.setLineWidth(2)
                c.line(margin, height - 60, width - margin, height - 60)
            ...
            buffer.seek(0)
            msg = EmailMessage()
            msg['Subject'] = f"{comp} - Outward Entry Report"
            msg['From'] = config.Email
            msg['To'] = email
            msg.set_content("Attached is your otward entry PDF from Office Hub.")

            # Attach PDF
            msg.add_attachment(buffer.read(), maintype='application', subtype='pdf', filename='outward-Entry.pdf')
    ...
    def employee_remove_screen(self):
        self.bldr.get_screen("delete").manager.current = "delete"

    def delete_employee(self):
        emp = self.bldr.get_screen("dele").ids.dele_label.text
        print(emp)
        with open("user_data.json", "r") as json_file:
            data = json.load(json_file)
        email = data.get("email")
        self.bldr.get_screen("dele").ids.dele_spinner.active = True
        # self.bldr.get_screen("signup").ids.sign_spinner.active = True
        storage_thread = threading.Thread(
            target=self.delete_employee_from_firestore,
            args=(email, emp),
            daemon=True  # Ensures thread exits when app closes
        )
        storage_thread.start()
    ...
    def delete_employee_from_firestore(self, email, emp_name):
        ...
        try:
            print("Your deleting method gone here.")
        
        except Exception as e:
            print(f"Error deleting employee: {e}")
            self.bldr.get_screen("dele").ids.dele_spinner.active = False




    def deleted_success(self, dt=None):
        self.bldr.get_screen("delete").manager.current = "delete"
